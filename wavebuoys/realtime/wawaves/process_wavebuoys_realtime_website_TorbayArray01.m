    ...
    if flag == 1
        ...
        if all(check)~=0        
            ...
            if SpotData.time(1)>archive_data.time(end)
                ...
                realtime_archive_mat(buoy_info, data); %aws
                realtime_backup_mat(buoy_info, data); %irds 
                realtime_archive_text(buoy_info, data, limit); %aws for website 

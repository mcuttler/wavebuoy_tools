...
buoy_info.serial = 'SPOT-31139C'; %spotter serial number, or just Datawell 
buoy_info.name = 'DongaraOffshore'; 
...
buoy_info.DeployLoc = 'DongaraOffshore';
buoy_info.DeployDepth = 23;
buoy_info.DeployLat = -29.198167;
buoy_info.DeployLon = 114.831950; 
            ...
            if ~isempty(idx_w)&~isempty(idx_t)
                ...
                if SpotData.time(1)>archive_data.time(end)
                    ...
                    if strcmp(buoy_info.DataType,'spectral')                        
                        [NS, NE, ndirec] = lygre_krogstad(data.a1(end,:),data.a2(end,:),data.b1(end,:),...
                            data.b2(end,:),data.varianceDensity(end,:));
                        make_MEM_plot(ndirec, data.frequency(end,:), NE, data.hsig(end,1),...
                            data.tp(end,1), data.dp(end,1), data.time(end,1), buoy_info)        
                    end

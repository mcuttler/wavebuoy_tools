...
function [log_message] = batch_realtime(buoy_info)
...
if strcmp(buoy_info.type,'sofar')==1     
    ...
    try
        ...
        flag = 0; 
        log_message = 'code failed on getting Spotter data - no new data or wrong API token'; 
    end                            
    ...
    if flag == 1
        ...
        try
            ...
            log_message = 'code failed on check or warning';
        end
        ...
        if all(check)~=0        
            ...
            try
                ...
                log_message = 'code failed loading archive data'; 
            end
            ...
            try
                ...
                log_message = 'code failed on indexing SpotData for new data'; 
            end
            ...
            if ~isempty(idx_w)&~isempty(idx_t)
                try
                    ...
                    log_message = 'code failed on cleaning up SpotData for new data';
                end                
                ...
                if SpotData.time(1)>archive_data.time(end)
                    ...
                    try
                        ...
                        log_message = 'code failed on running QAQC';
                    end
                    ...
                    try
                        ...
                        log_message = 'code failed on archiving or writing text file';
                    end
                    ...
                    if strcmp(buoy_info.DataType,'spectral')                        
                        try
                            ...
                            log_message = 'code failed on making MEM';
                        end                        
                    end
...
elseif strcmp(buoy_info.type,'datawell')==1
    ...
    try
        ...
        log_message = 'code failed on getting Datawell data'; 
    end        
    clear data; 
    ...
    if all(check)~=0
        if ~isempty(archive_data)
            if size(dw_data.time,1)>size(archive_data.time,1)
                ...
                try
                    ...
                    log_message = 'code failed on archiving or making text file';
                end
                ...
                try
                    ...
                    log_message = 'code failed updating buoys.csv'; 
                end            
            end
        end
    else
        ...
        try
            ...
            log_message = 'code failed archiving or making text';
        end
        ...
        if strcmp(buoy_info.DataType,'spectral')        
            try
                ...
                log_message = 'code failed making MEM'; 
            end
        end

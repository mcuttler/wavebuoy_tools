...
function [] = locations_to_IMOS_nc(locations, outpathNC, buoy_info, globfile, varsfile); 
...
for i = 1:size(tdata,1)
    ...
    disp(['Saving GPS ' datestr(tstart,'mmm yyyy')]); 
    
    %output for BULK PARAMETERS     
    idx_bulk = []; 
    idx_bulk = find(locations.time>=tstart&locations.time<tend); 
    
    filenameNC = [outpathNC '\' buoy_info.name '_' buoy_info.DeployLoc '_' datestr(tstart,'yyyymm') '_gps.nc'];             
    ...
    dimname = 'TIME';
    dimlength = size(locations.time(idx_bulk),1);
    ...
    for ii = 1:m    
        ...
        if strcmp(varinfo{1,1}{ii,1},'time')
            imos_time = locations.time(idx_bulk) - datenum(1950,1,1,0,0,0); 
            netcdf.putVar(ncid, varid, imos_time); 
        else
            netcdf.putVar(ncid, varid, locations.(varinfo{1,1}{ii,1})(idx_bulk));
        end

...
if isempty(rw) | (length(rw)<windows*0.33 & length(find(heave==0))/length(heave)<0.1)
   ...
   %CALCUALTE WAVE PARAMETERS AND ORGANIZE OUTPUT
    ...
    elseif strcmp(type,'enu') %enu = velocity
        ...
        SpIG = trapz(freq(freq>=info.fminIG & freq<info.fminSS), Scorr(freq>=info.fminIG & freq<info.fminSS)); 
        SpSS = trapz(freq(freq>=info.fminSS & freq<info.fmaxSS), Scorr(freq>=info.fminSS & freq<info.fmaxSS));        
        out.HsigSS = 4*sqrt(SpSS); 
        out.HsigIG = 4*sqrt(SpIG); 
        out.Hrms=sqrt(8*SpTot);
        out.HrmsSS = sqrt(8*SpSS); 
        out.HrmsIG = sqrt(8*SpIG); 
        out.f=freq;
        out.spec1D=Scorr;
        out.a1=a1;
        out.a2=a2;
        ...
        out.segments_used=windows-length(rw);
        if isfield(info,'fmaxSea')
            SpSea = trapz(freq(freq>=info.fmaxSS & freq<info.fmaxSea), Scorr(freq>=info.fmaxSS & freq<info.fmaxSea));     
            out.HsigSea = 4*sqrt(SpSea); 
            out.HrmsSea = sqrt(8*SpSea); 

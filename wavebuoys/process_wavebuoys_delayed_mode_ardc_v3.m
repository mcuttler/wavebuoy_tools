...
for b = 1:size(buoy_metadata,1)
    ...
    %% set spectral analysis settings 
            ...
            %set up time - kill first and last day of data, modify this if have
            %correct deploy/retrieve dates/times. Or, could use watch circle code on
            %the locations (but need to re-write this to work in lat/lon
            % tstart = dateshift(buoy_xyz.Time(1)+days(1),'start','day'); 
            % tend = dateshift(buoy_xyz.Time(end),'start','day'); 
        ...
        data = bulkparams; clear bulkparams
    ...
    %displacements
    fields = buoy_xyz.Properties.VariableNames;  
    for i = 1:length(fields)
        if strcmp(fields{i},'Time')
            data.disp_time = buoy_xyz.(fields{i}); 
        else
            data.(fields{i}) = buoy_xyz.(fields{i}); 
        end
    end
    % clear buoy_xyz;   
 %Datawell DWR4 
elseif strcmp(buoy_info.type,'datawell')==1
   %create blank array for output
    ...
    %use mapping toolbox distance function to calculate points outside
    %watch_circle
    clear dum_distance
    ...
    fields = fieldnames(data); 
    for i = 25:length(fields)
        if ~strcmp(fields{i},'frequency') & ~contains(fields{i},'limits') & ~strcmp(fields{i},'x') & ~strcmp(fields{i},'y') & ~strcmp(fields{i},'z')
            if iscell(data.(fields{i}))
                data.(fields{i}) = data.(fields{i})(1,ind); 
            else
                data.(fields{i}) = data.(fields{i})(ind,:); 
            end
        end
    end

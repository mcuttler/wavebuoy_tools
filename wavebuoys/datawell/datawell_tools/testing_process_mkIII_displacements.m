...
for i = 1:50; %size(dum,1); 
    ...
    if ~strcmp(dum(i).name(1:3),'TMP')
        ...
        %read in data
        tmp = importdata(fullfile(dum(i).folder, dum(i).name));              
        rdt.file_size(cnt,1) = size(tmp.data,1); 
        
        %figure out start time based on bulk parameters time stamps -
        %assume first displacemenet in RDT is first sample from that day in
        %SDT 
        ...
        tstart = sdt.time(tind); 
        %figure out end date based on number of samples
        tend = tstart+((size(tmp.data,1)/1.28)*(1/(60*60*24))); 

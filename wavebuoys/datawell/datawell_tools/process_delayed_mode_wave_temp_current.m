...
function [data] = process_delayed_mode_wave_temp_current(buoy_info, E, theta, s, m2, n2, spec_params, wave_time,gps,gps_time, TC, temp_time, data); 
...
for ii = 1:m
    ...
    [NS, NE, ndirec] = lygre_krogstad_MC(a1,a2,b1,b2,dumE,1);
    ...
    data.ndirec = ndirec;
%     data.spec1D = [data.spec1D; spec1D'];
    data.spec2D = cat(3,data.spec2D,NE);
...
data.gps_pos=[gps(:,3)*180/pi gps(:,4)*180/pi];

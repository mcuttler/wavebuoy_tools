...
%analysed to determine whether suspect or fail
    ...
    function [qc_flag_watch, watch_circle_flag, buoy_info] = qaqc_watch_circle(buoy_info, dlat, dlon)
    
    qc_flag_watch = ones(size(dlat,1),1); 
    ...
    for i = 1:size(dlat,1)     
        if dlat(i) > -180
            dum_distance(i,1) = distance(buoy_info.DeployLat, buoy_info.DeployLon, dlat(i), dlon(i),wgs84); 
        else
    ...
    if ~isempty(ind)
        ...
        qc_flag_watch(dum_distance > buoy_info.watch_circle,1) = 3; 
        % overwrite any suspect in previous line that are bigger than threshold 
        qc_flag_watch(dum_distance > buoy_info.watch_circle*buoy_info.watch_circle_fail,1) = 4; 
    end

...
function [QCFlag] = qartod_19_bulkparams_range(in, fail_sus, type)

for ii = 1:size(in.data,1)
    if in.data(ii) > in.data_max | in.data(ii) < in.data_min
        QCFlag(ii,1) = fail_sus; 
        QCFlag(ii,2) = type; 
    else
        QCFlag(ii,1) = 1; 
        QCFlag(ii,2) = 0; 

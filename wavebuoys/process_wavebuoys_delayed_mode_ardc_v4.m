...
for b = 1:size(buoy_metadata,1)
    ...
    if strcmp(buoy_info.type,'sofar')==1        
        ...
        %calculate watch circle in two iterative steps to try to catch
        %small hops or inaccuracies in the mooring build or errors in
        %position
        ...
        %average GPS data to daily and use that for calculations of watch circle
        % gps_daily = retime(gps,'daily',@nanmean); 
        ...
        %if too much out of radius, try to re-calculate with some more
        %assumptions 
        if out_of_radius >= buoy_info.out_of_radius_tolerance
            ...
            if out_of_radius >= buoy_info.out_of_radius_tolerance
                %if still too much, flag this as potentially drifting or
                %big movements 
                questionableData = 1;   %use this to flag later steps to proceed or not               
            else    
                questionableData = 0;
                gps = gps(ind,:); 
        ...
        if questionableData<1 %only do spectral analysis if passed watch circle QC 
            for i = 1:length(dt)-1
...
fprintf(flog, [buoy_info.name ' failed the watch circle QAQC tests and needs a closer look.']); 
fclose(flog); 

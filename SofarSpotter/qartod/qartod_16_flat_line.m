...
%check each input variable and then at end keep highest QC flat
QCFlag_suspect  = zeros(length(in.WVHGT),4);
%
for ii=1:length(in.WVHGT)
    %check if first n values are 'equal' within tolerance
    if ii<=in.rep_suspect
        checkWVHGT = diff(in.WVHGT(ii:in.rep_suspect-1));                
        checkWVPD = diff(in.WVPD(ii:in.rep_suspect-1));        
        checkWVDIR = diff(in.WVDIR(ii:in.rep_suspect-1));         
        checkWVSP = diff(in.WVSP(ii:in.rep_suspect-1));
...
for ii=1:length(in.WVHGT)
    %check if first n values are 'equal' within tolerance
    if ii<=in.rep_fail
        checkWVHGT = diff(in.WVHGT(ii:in.rep_fail-1));                
        checkWVPD = diff(in.WVPD(ii:in.rep_fail-1));        
        checkWVDIR = diff(in.WVDIR(ii:in.rep_fail-1));         
        checkWVSP = diff(in.WVSP(ii:in.rep_fail-1));
